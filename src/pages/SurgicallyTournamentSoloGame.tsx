...
import './SurgicallyTournamentSoloGame.css'
...
setShowRecord(true)
  ...
  /* ========================================
     TOURNOI : UNE ERREUR = ELIMINATION
     Aucun système de trois vies ici.
  ======================================== */
  function loseLife() {
    if (
      gameStateRef.current !== 'playing' ||
    ...
    // Verrouiller immédiatement pour empêcher un second contact
    // ou une extraction de rapporter des points après l'erreur.
    missionLockedRef.current = true
    deadlineRef.current = 0
    activePointerIdRef.current = null
    pinceActiveRef.current = false
    carriedOrganRef.current = null
    recordPausedRef.current = false
    resetPince()
    setFeedback('AÏE !')
    setPatientExpression('grimace')
    stopBackgroundMusic()
    ...
    // Le classement de la manche est enregistré par l'effet gameover.
  ...
  function returnToMenu() {
    ...
    navigate(
      getSoloTournamentReturnPath('surgically-insane')
    )
  }
  ...
  /* Manche de tournoi : 45 secondes au maximum, élimination immédiate sur erreur.
     La pause ne consomme pas le temps restant. */
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      previous = now
      if (manualPausedRef.current || recordPausedRef.current) return
      roundTimeRef.current = Math.max(0, roundTimeRef.current - delta)
      setRoundSecondsLeft(Math.ceil(roundTimeRef.current))
      if (roundTimeRef.current <= 0 && gameStateRef.current === 'playing') {
        missionLockedRef.current = true
        deadlineRef.current = 0
        stopBackgroundMusic()
        changeGameState('gameover')
  ...
  useEffect(() => {
    ...
    recordSoloRound('surgically-insane', scoreRef.current)
  }, [gameState])
  ...
  return (
    ...
    <section
      className={
        `surgically-page surgically-tournament-solo surgically-state-${gameState}`
      }
    >

      <TournamentSoloPlayersHud active={gameState === 'playing'} humanScore={score} />
      {gameState === 'playing' && (
        <div className="surgically-tournament-timer" role="timer" aria-label="Temps restant">
          ⏱ {roundSecondsLeft}s
        </div>
      )}
      ...
      {gameState === 'ready' && (
        ...
        <div className="surgically-start-screen">
          ...
          <p className="surgically-start-description">
            {tr("Extrais les bons organes sans toucher les bords. Une seule erreur et tu es éliminé !")}</p>
